...
ggplot() + geom_point(data=comprehensive_stats, aes(x=P., y=PP.))
```
Next, penalty kill percentage and regular season wins

```{r}
ggplot() + geom_point(data=comprehensive_stats, aes(x=P., y=PK.))
```
...
ggplot() + geom_point(data=playoffs, aes(x=W, y=PP.))
```
```{r}
ggplot() + geom_point(data=playoffs, aes(x=W, y=PK.))
```
